...
class IncomeExpenseTracker:
    def __init__(self, root):
        ...
        # Report Button
        tk.Button(root, text="Generate Report", command=self.generate_report).pack()
    ...
    def generate_report(self):
        ...
        transactions = tracker_db.load_transactions()
        categories, income_values, expense_values = tracker_logic.prepare_report_data(transactions)

        fig, ax = plt.subplots()
        ax.bar(categories, income_values, color="green", label="Income")
        ax.bar(categories, expense_values, color="red", label="Expense")

        ax.set_title("Income vs Expense Report")
        ax.legend()
